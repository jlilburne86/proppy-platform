...
def build_page_html(fm: dict, body_html: str, nav_html: str, slug: str, raw_md: str) -> str:
  ...
  <a href="../resources.html#{anchor}" class="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-sm font-semibold">
    ...
    def see_also_links(this_slug: str, cat: str) -> str:
        import os
        items = []
        for pth in ART_MD_DIR.glob('*.md'):
            if pth.stem == this_slug:
                continue
            try:
                txt2 = pth.read_text(encoding='utf-8', errors='ignore')
            except Exception:
                continue
            fm2 = parse_front_matter(txt2)[0]
            cat2 = (fm2.get('category','') or '').strip('[]').split(',')[0].strip()
            status = (fm2.get('publish_status','') or '').lower()
            if status != 'published':
                continue
            if cat2 == cat or pillar == CAT_TO_PILLAR.get(cat2, cat2):
                items.append((pth.stat().st_mtime, fm2.get('title', pth.stem), pth.stem))
        items.sort(reverse=True)
        items = items[:2]
        if not items:
            return ""
        links = " • ".join([f'<a class=\"text-primary hover:underline\" href=\"{s}.html\">{title}</a>' for _, title, s in items])
        return f'<section class=\"max-w-3xl mx-auto px-6 py-2\"><p class=\"text-sm\">See also: {links}</p></section>'
