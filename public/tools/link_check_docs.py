...
def main():
    missing = []
    files = list(DOCS.glob('*.html')) + list((DOCS / 'articles').glob('*.html')) + list((DOCS/'property').glob('*.html'))
    for f in files:
        ...
        for u in extract_urls(html):
            ...
            if not dest.exists():
                missing.append((str(f.relative_to(DOCS)), u))
    if missing:
        print('Broken docs links/files:')
